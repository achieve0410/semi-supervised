...
bModel = RandomForestClassifier(criterion='entropy', n_estimators=10, n_jobs=2, random_state=1)
bModel.fit(labeled_x, labeled_y.ravel())

## normalization x
normalization(unlabeled_x)

## predict the answer
npred = bModel.predict(unlabeled_x)
...
aModel = RandomForestClassifier(criterion='entropy', n_estimators=10, n_jobs=2, random_state=1)
aModel.fit(new_labeled_x, new_labeled_y.ravel())

## predict the answer using new models
apred = aModel.predict(unlabeled_x)
apred_y = makearray(apred)
# print(apred_y)

## predict the answer using old models
bpred = bModel.predict(unlabeled_x)
